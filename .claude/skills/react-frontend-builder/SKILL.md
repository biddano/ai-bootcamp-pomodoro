...
If there is no existing frontend architecture root, create a clean directory at the root of the workspace named ${filepath}. If an application frame exists, target that directory explicitly for all downstream file mutations, package installs, and compilation verifications.
...
* Run the initialization scaffolding command using Vite with the TypeScript variant:
  ```bash
  npm create vite@latest ${filepath} -- --template react-ts